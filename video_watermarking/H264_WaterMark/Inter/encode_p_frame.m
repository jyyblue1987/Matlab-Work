function [Seq_r,bits_frame] = encode_p_frame(Seq,Quant,flag,block_size,change)
...
if block_id==BlockForEncode&&water_value>0
    pred_dir=1;
    if mvx<0 pred_dir = 2;
    else pred_dir = 1;
    end
    if pred_dir==water_value
        
    else
        [mvx,mvy,rec,bits,ssd] = motion_comp_block_part(bs,bs,mr,i,j,water_value);        
    end
...
function [mvx,mvy,rec,bits_b,ssd] = motion_comp_block_part(bsx,bsy,mr,i,j, part)
...
switch part
    case 0
        swr = mirror_pad(extract_object(Sr(:,:,frame-1),[j-mr i-mr bsx+2*mr bsy+2*mr],[1 1 width height]));    
    case 1
        swr = mirror_pad(extract_object(Sr(:,:,frame-1),[j+(bsx/2) i-mr bsx+2*mr bsy+2*mr],[1 1 width height]));  
        x_diff = mr+(bsx/2);    
    case 2
        swr = mirror_pad(extract_object(Sr(:,:,frame-1),[j-(bsx/2)-2*mr i-mr bsx+2*mr bsy+2*mr],[1 1 width height]));    
        x_diff = -mr-(bsx/2);    
end
