...
function [Side_Info, blk_im]=Encode(Side_Info,im_old, im_new,recons_im,i,j,rblk, cblk,N,change)
...
im_new1 = double(padarray(im_new,[W/2 W/2],'replicate'));
    ...
    if err_tp<=err_lt||change==2
        %do Inter prediction itself
        Side_Info(rblk,cblk).prediction='Inter';
        Side_Info(rblk,cblk).Block_Size=N;
        
        if change==2
            motion(1)=-1*(abs(motion(1))+3);%Watermark=1
        elseif change==1
            motion(1)=+1*(abs(motion(1))+3);%Watermark=0
        end
